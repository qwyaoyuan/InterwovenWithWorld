...
public class MoveManager : IInput
{
    ...
    private MoveManager()
    {
        ...
        GameState.Instance.Registor<IPlayerState>(CallbackPlayerState);
    }
    ...
    private void CallbackGameState(IGameState iGameState, string fieldName)
    {
        ...
        {
            if (iGameState.GameRunType != EnumGameRunType.Safe && iGameState.GameRunType != EnumGameRunType.Safe)
            {
                iAnimatorState.AnimatorMoveSpeed = 0;
            }
        }
    }
    ...
    /// <param name="fieldName"></param>
    private void CallbackPlayerState(IPlayerState iPlayerState, string fieldName)
    {
        //检测到强制移动
